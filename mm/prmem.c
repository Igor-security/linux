...
unsigned long __weak memset_user(void __user *to, int c, unsigned long n)
{
	...
	for (i = 0; i < n; i++)
		put_user((char)c, (char *)(i + (unsigned long)to));
	return n;
}
#endif
 ...
 *
 * NOTE:
 * wr_memcopy is used in cases where there is an expectation that the copy
 * of aligned, native data types will be atomic. Such assumption
 * simplifies the handling of pointers (unsigned long), for example.
 * Before enabling write rare on an architecture, this assumption must be
 * verified.
 */
