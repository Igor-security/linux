...
/*
 * This is generic code, relying on the architecture code to provide:
...
/**
 * wr_memcpy() - copies size bytes from q to p
...
void *wr_memcpy(void *p, const void *q, __kernel_size_t size)
{
	...
	if (WARN_ONCE(!is_wr_after_init(p, size), "Invalid WR range."))
		return p;

	if (unlikely(wr_mem_is_writable()))
		return memcpy(p, q, size);

	wr_poking_addr = __wr_addr(p);
	local_irq_disable();
...
void *wr_memset(void *p, int c, __kernel_size_t len)
{
	...
	if (WARN_ONCE(!is_wr_after_init(p, len), "Invalid WR range."))
		return p;
