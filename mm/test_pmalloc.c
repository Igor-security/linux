...
static bool test_pmalloc_rare_write_array(void)
{
	...
	pr_notice("Testing pmalloc_rare_write");
...
static bool test_pmalloc_rare_write_char(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_char");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_short(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_short");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_ushort(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_ushort");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_int(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_int");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_uint(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_uint");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_long(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_long");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_ulong(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_ulong");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_longlong(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_longlong");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
static bool test_pmalloc_rare_write_ulonglong(void)
{
	...
	retval = false;
	pr_notice("Testing rare_write_ulonglong");
	pool = pmalloc_create_pool(PMALLOC_RW);
	if (WARN(!pool, "Failed to create pool"))
		return false;
...
{
	return test_pmalloc_rare_write_char() &&
...
static bool test_illegal_rare_write_ro_pool(void)
{
	...
	pr_notice("Illegal rare_write to R/O pool - it should WARN");
	pool = pmalloc_create_pool(PMALLOC_RO);
	if (WARN(!pool, "Failed to create pool"))
		return false;
	...
	*var_ptr = 0xA5;
	...
	if (WARN(pmalloc_rare_write_int(pool, var_ptr, 0x5A),
		...
		goto destroy_pool;
	retval = true;
	pr_notice("Illegal rare_write to RO pool rejected");
destroy_pool:
	pmalloc_destroy_pool(pool);
	return retval;
}
...
static bool test_illegal_rare_write_static_rare_write_mem(void)
{
	...
	*dummy = 1;
	pmalloc_protect_pool(pool);
	pr_notice("Illegal rare_write to static memory - it should WARN");
	...
	if(WARN(write_result || rare_write_data != 0xA5,
		...
		return false;
	pr_notice("Illegal rare_write to static memory rejected");
	return true;
}
...
static bool test_illegal_rare_write_ro_after_init(void)
{
	...
	*dummy = 1;
	pmalloc_protect_pool(pool);
	pr_notice("Illegal rare_write to ro_after_init - it should WARN");
	...
	if(WARN(write_result || ro_after_init_data != 0xA5,
		...
		return false;
	pr_notice("Illegal rare_write to ro_after_init memory rejected");
	return true;
}

static bool test_illegal_rare_writes(void)
{
	return test_illegal_rare_write_ro_after_init() &&
