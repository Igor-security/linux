...
static bool test_pmalloc_wr_memset(void)
{
	struct pmalloc_pool *pool;
	char *region;
	unsigned int i;
	...
	for (i = 0; i < REGION_SIZE; i++)
		if (WARN(region[i], "Failed to memset wr memory"))
			goto destroy_pool;
	retval = !pmalloc_wr_memset(region + INSERT_OFFSET, 1, INSERT_SIZE);
	if (WARN(retval, "pmalloc_wr_memset failed"))
		goto destroy_pool;
	for (i = 0; i < REGION_SIZE; i++)
		if (i >= INSERT_OFFSET &&
	...
	retval = true;
	pr_success("pmalloc_wr_memset");
destroy_pool:
	pmalloc_destroy_pool(pool);
	return retval;
}

static bool test_pmalloc_wr_strdup(void)
{
...
static bool test_pmalloc_wr_copy(void)
{
	struct pmalloc_pool *pool;
	char *region;
	char *mod;
	...
	retval = !pmalloc_wr_copy(region + INSERT_OFFSET, mod, INSERT_SIZE);
...
static bool test_pmalloc_wr_char(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = (char)0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_char(array + TEST_ARRAY_TARGET, (char)0x5A),
...
static bool test_pmalloc_wr_short(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = (short)0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_short(array + TEST_ARRAY_TARGET, (short)0x5A),
...
static bool test_pmalloc_wr_ushort(void)
{
	...
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_ushort(array + TEST_ARRAY_TARGET,
				    (unsigned short)0x5A),
...
static bool test_pmalloc_wr_int(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = 0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_int(array + TEST_ARRAY_TARGET, 0x5A),
...
static bool test_pmalloc_wr_uint(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = 0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_uint(array + TEST_ARRAY_TARGET, 0x5A),
...
static bool test_pmalloc_wr_long(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = 0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_long(array + TEST_ARRAY_TARGET, 0x5A),
...
static bool test_pmalloc_wr_ulong(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = 0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_ulong(array + TEST_ARRAY_TARGET, 0x5A),
...
static bool test_pmalloc_wr_longlong(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = 0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_longlong(array + TEST_ARRAY_TARGET, 0x5A),
		 "Failed to alter long variable"))
		goto destroy_pool;
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		if (WARN(array[i] != (i == TEST_ARRAY_TARGET ? 0x5A : 0xA5),
...
static bool test_pmalloc_wr_ulonglong(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = 0xA5;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_ulonglong(array + TEST_ARRAY_TARGET, 0x5A),
...
static bool test_pmalloc_wr_ptr(void)
{
	...
	for (i = 0; i < TEST_ARRAY_SIZE; i++)
		array[i] = NULL;
	pmalloc_protect_pool(pool);
	if (WARN(!pmalloc_wr_ptr(array + TEST_ARRAY_TARGET, array),
