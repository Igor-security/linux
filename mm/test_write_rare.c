...
static bool test_wr_memset(void)
{
	...
	wr_memset(array, 0x69, 24);
//	WARN(memtst(array, 0x69, sizeof(unsigned long) * 3), "MemSet Fail");
	pr_info("Values: 0x%016lx 0x%016lx 0x%016lx %u",
		*(unsigned long *)array, *(1 + (unsigned long *)array),
		*(2 + (unsigned long *)array), sizeof(unsigned long));
