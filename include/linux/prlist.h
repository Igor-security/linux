...
void prlist_del_entry(struct prlist_head *entry)
{
	...
	pmalloc_wr_ptr(&entry->next, LIST_POISON1);
	pmalloc_wr_ptr(&entry->prev, LIST_POISON2);
}
		       ...
		       struct prhlist_node *first)
{
	if (is_static(head))
		...
		pmalloc_wr_ptr(&head->first, first);
}
		       ...
		       struct prhlist_node **pprev)
{
	if (is_static(node))
		...
		pmalloc_wr_ptr(&node->pprev, pprev);
}
		       ...
		       struct prhlist_node *prev)
{
	if (is_static(node->pprev))
		...
		pmalloc_wr_ptr(node->pprev, prev);
}
...
{
	pmalloc_wr_ptr(&node->next, NULL);
	pmalloc_wr_ptr(&node->pprev, NULL);
}
...
{
	__prhlist_del(node);
	pmalloc_wr_ptr(&node->next, LIST_POISON1);
	pmalloc_wr_ptr(&node->pprev, LIST_POISON2);
}
