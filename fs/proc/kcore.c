...
kclist_add_private(unsigned long pfn, unsigned long nr_pages, void *arg)
{
	 ...
	 * if we need to trim
	 */
	if (VMALLOC_RW_START > ent->addr) {
		if (VMALLOC_RW_START - ent->addr < ent->size)
			ent->size = VMALLOC_RW_START - ent->addr;
	}
...
static int __init proc_kcore_init(void)
{
	...
	/* Store vmalloc area */
	kclist_add(&kcore_vmalloc, (void *)VMALLOC_RW_START,
		VMALLOC_RW_SIZE, KCORE_VMALLOC);
	add_modules_range();
